...
由于 MySQL 是开源免费并且比较成熟的数据库，因此MySQL 被大量使用在各种系统中。任何人都可以在 GPL(General Public License) 的许可下下载并根据个性化的需要对其进行修改。MySQL 的默认端口号是**3306**。

## MySQL 基础架构

下图是 MySQL 的一个简要架构图，从下图你可以很清晰的看到客户端的一条 SQL 语句在 MySQL 内部是如何执行的。

![img](http://qiniu.xinghe.fit/Interview/13526879-3037b144ed09eb88.png)
...
![查看 MySQL 提供的所有存储引擎](http://qiniu.xinghe.fit/Interview/image-20220510105408703.png)
...
![查看 MySQL 当前默认的存储引擎](http://qiniu.xinghe.fit/Interview/image-20220510105837786.png)

如果你只想查看数据库中某个表使用的存储引擎的话，可以使用 `show table status from db_name where name='table_name'`命令。

![查看表的存储引擎](http://qiniu.xinghe.fit/Interview/image-20220510110549140.png)
...
![img](http://qiniu.xinghe.fit/Interview/image-20220510090309427.png)
...
MyISAM 不支持，而 InnoDB 支持。毕竟 MyISAM 连行级锁都不支持。MVCC 可以看作是行级锁的一个升级，可以有效减少加锁操作，提高性能。
...
大多数时候我们使用的都是 InnoDB 存储引擎，在某些读密集的情况下，使用 MyISAM 也是合适的。不过，前提是你的项目不介意 MyISAM 不支持事务、崩溃恢复等缺点（可是我们一般都会介意啊！）。
...
**查询缓存不命中的情况：（1）**因此任何两个查询在任何字符上的不同都会导致缓存不命中。
...
![事务示意图](http://qiniu.xinghe.fit/Interview/%E4%BA%8B%E5%8A%A1%E7%A4%BA%E6%84%8F%E5%9B%BE.png)
...
![数据库事务示意图](http://qiniu.xinghe.fit/Interview/%E6%95%B0%E6%8D%AE%E5%BA%93%E4%BA%8B%E5%8A%A1%E7%A4%BA%E6%84%8F%E5%9B%BE.png)

另外，关系型数据库（例如：`MySQL`、`SQL Server`、`Oracle` 等）事务都有 **ACID** 特性：

![ACID](http://qiniu.xinghe.fit/Interview/ACID.png)
...
![AID->C](http://qiniu.xinghe.fit/Interview/AID-C.png)
...
![img](http://qiniu.xinghe.fit/Interview/20210526162552353.png)
...
不过， 之外的其他隔离级别可能也需要用到锁机制，就比如 REPEATABLE-READ 在当前读情况下需要使用加锁读来保证不会出现幻读。
...
![img](http://qiniu.xinghe.fit/Interview/image-20220511171419081.png)
